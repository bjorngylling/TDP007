...
class ConstraintParser < Parser
    ...
    @parser=super("constraints") do
      ...
      start :statement do
        ...
        match(:expr, '=', :term) do |lh, _, rh|         	
          ...
          x = @connectors.values.reject do |conn|
  ...
  def get_connectors(a,op,b)
  	# Added check here to make sure both conn_a and conn_b are connectors and not constraints
  	conn_a=get_connector(a)
    conn_b=get_connector(b)
  ...
  def replace_conn(lh,rh)
    ...
    conn,exp=[nil,nil]
    if rh.is_a?(ArithmeticConstraint)
      exp,conn=rh,lh_conn
      @connectors.delete(rh_conn.name)
    elsif lh.is_a?(ArithmeticConstraint) # elsif instead of "else if"
			exp,conn=lh,rh_conn
